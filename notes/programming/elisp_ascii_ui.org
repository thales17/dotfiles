...
#+begin_src elisp
  ...
  (defun ajr--draw-room (room)
    (let ((x (ajr-2d--room-x room))
	  (y (ajr-2d--room-y room))
	  (w (ajr-2d--room-w room))
	  (h (ajr-2d--room-h room)))
      (goto-char (+ (* y 81) x 1))
      (insert (format "+%s+\n" (make-string w ?.)))
      (mapcar (lambda (n)
		(insert (format "%s|%s|\n"
  ...
  (defun ajr-ascii-room ()
    (interactive)
    (let ((2d-buffer (get-buffer-create "*ascii-room*")))
      (with-current-buffer 2d-buffer
	(ajr-ascii-room-mode)
	(setq buffer-read-only nil)
	(ajr-2d--clear)
	(ajr-2d--draw-room (ajr-2d-make-room 16 5 15 10))
	(setq buffer-read-only t)
	(pop-to-buffer 2d-buffer))))
