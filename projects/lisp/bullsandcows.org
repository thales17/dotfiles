...
#+begin_src lisp
  (defun shuffle-list (l)
	...
	(when (>= i (length l))
	  (return))
	(rotatef (nth (random (length l)) l)
		 (nth (random (length l)) l) l)
	(set i (1+ i)))))
#+end_src
...
#+begin_src lisp
  (defun run-cows-solver ()
    (let ((digits *code-digits*)
	  (i 0))
      (loop
	(when (>= i (length digits))
	  (return))
	(when (= (score-total (guess-code (sublist i))) 0)
	  (print "zero detected!"))
	(print (list (sublist i) (guess-code (sublist i))))
	(setf i (1+ i)))
      (print (shuffle-list digits))))
#+end_src
